...
- Bankhoes Turso database `bankhoes-bi-data-zanderbmc.aws-eu-west-1.turso.io`, approved by Zander for this scanner only to create, verify, read, and write `ampere_*` tables. Do not inspect, query, mutate, migrate, or couple to any other table; do not create/revoke tokens or change database/account settings.
