...
- `GET /orders` and `GET /shipments` use 1-based `page` pagination with up to 50 records per page. An omitted or empty collection proves the boundary. Orders continue to that boundary. Shipments are documented newest-first, so the scanner validates descending timestamps and stops after either that boundary or a page crossing the previous Amsterdam cutoff; this proves the complete operational window without reading the three-month shipment archive.
